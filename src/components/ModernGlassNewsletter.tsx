...
import { Envelope, CheckCircle, ArrowRight, Users, TrendUp, Medal } from '@phosphor-icons/react';
...
export const ModernGlassNewsletter: React.FC<ModernGlassNewsletterProps> = ({ onSubscribe, isLoading }) => {
 ...
 <div 
 ref={containerRef}
 className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-purple-600 via-pink-600 to-red-600 p-1 transition-all duration-300 hover:"
 style={{
 background: `
 ...
 <Medal className="w-5 h-5 text-white" weight="duotone" />
 </div>
 <div>
 ...
 isFocused 
 ? 'border-white/50 bg-white/30 shadow-white/20' 
 ...
 <ArrowRight className="w-5 h-5 transition-transform duration-300 group-hover:translate-x-1" weight="duotone" />
 </>
 )}
 </div>
 </button>
